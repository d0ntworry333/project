...
async def show_training_status(update: Update, context: ContextTypes.DEFAULT_TYPE, session):
    ...
    if update.callback_query:
        # Для callback_query нельзя использовать ReplyKeyboardMarkup в edit_message_text
        # Поэтому редактируем сообщение без клавиатуры, а затем отправляем новое с клавиатурой
        await update.callback_query.edit_message_text(status_text)
        await update.callback_query.message.reply_text(
            "💪 Используйте клавиатуру для управления тренировками:",
            reply_markup=reply_markup
        )
    else:
